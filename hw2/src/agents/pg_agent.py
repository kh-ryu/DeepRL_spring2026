...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        discount_factor = np.power(self.gamma, np.arange(len(rewards)))
        discounted_sum = np.sum(discount_factor * np.array(rewards))
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in range(len(rewards)):
            discounted_returns.append(np.sum(discount_factor[: len(rewards) - t] * np.array(rewards[t:])))
        return discounted_returns
    ...
    def _estimate_advantage(
        ...
        if self.normalize_advantages:
            ...
            std = np.std(advantages) + 1e-8  # Add small constant
